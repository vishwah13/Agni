...
namespace agni
{
	namespace editor
	{
		...
		EditorManager::EditorManager(AgniEngine& engine) : m_engine(engine) {}
		...
		void EditorManager::init()
		{
			...
			// Create inspector
			m_inspector =
			std::make_unique<ECSInspector>(*this,
			...
			m_sceneSerializer =
			std::make_unique<agni::scene::SceneSerializer>(m_engine);
		...
		void EditorManager::render()
		{
			...
			// Render inspector windows (controlled by EditorUI visibility
			// flags)
			...
			for (auto it = m_activeLoads.begin(); it != m_activeLoads.end();)
			{
				...
				if (handle->gpuUploadComplete)
				{
					if (handle->result)
					{
						...
						std::string filename =
						handle->filePath.filename().string();
						m_loadedAssets[filename] = handle->result;

						// Also store in renderer's loaded scenes so GPU
						// resources stay alive
						m_engine.m_renderer.getLoadedScenes()[filename] =
						handle->result;

						AGNI_PRINT("[Editor] Asset loaded: {}\n", filename);

						// TODO: Optionally convert to ECS entities
						// automatically
		...
		void EditorManager::setupShortcuts()
		{
			...
			m_inputManager->registerShortcut({SDLK_DELETE, false, false, false},
			                                 [this]()
			                                 { deleteSelectedEntity(); });

			// Escape - deselect entity
			m_inputManager->registerShortcut(
			{SDLK_ESCAPE, false, false, false},
			[this]() { setSelectedEntity(NULL_ENTITY); });

			// Ctrl+D - duplicate entity
			m_inputManager->registerShortcut({SDLK_D, true, false, false},
			                                 [this]()
			                                 { duplicateSelectedEntity(); });

			// Ctrl+N - new scene
			m_inputManager->registerShortcut({SDLK_N, true, false, false},
			                                 [this]() { newScene(); });

			// Ctrl+O - open scene
			m_inputManager->registerShortcut({SDLK_O, true, false, false},
			                                 [this]() { openScene(); });

			// Ctrl+S - save scene
			m_inputManager->registerShortcut({SDLK_S, true, false, false},
			                                 [this]() { saveScene(); });

			// Ctrl+Shift+S - save scene as
			m_inputManager->registerShortcut({SDLK_S, true, true, false},
			                                 [this]() { saveSceneAs(); });

			// Ctrl+Z - undo
			m_inputManager->registerShortcut({SDLK_Z, true, false, false},
			                                 [this]() { undo(); });
		...
		void EditorManager::createEntity(EntityType       type,
		                               ...
		                               const ImVec2&                dropPos)
		{
			...
			if (ext == ".gltf" || ext == ".glb")
			{
				...
				if (m_assetBrowser && m_assetBrowser->isVisible())
				{
					...
					ImGuiWindow* window =
					ImGui::FindWindowByName("Asset Browser");
					if (window)
					{
				...
				else
				{
					...
					AGNI_PRINT("[Editor] Starting async load: {}\n",
					           path.string());
					auto handle =
					m_engine.m_assetLoader.loadGltfAsync(&m_engine, path);
		...
		const std::filesystem::path& EditorManager::getCurrentScenePath() const
		{
			...
			return m_sceneSerializer ? m_sceneSerializer->getCurrentScenePath()
			                         : empty;
		}
		...
		static void openSceneCallback(void*              userdata,
		...
		static void saveSceneAsCallback(void*              userdata,
		                                const char* const* filelist,
		                                int                filter)
		{
			(void) filter;
		...
		void EditorManager::openScene()
		{
			...
			SDL_DialogFileFilter filters[] = {{"Scene Files", "json"},
			                                  {"All Files", "*"}};

			SDL_ShowOpenFileDialog(openSceneCallback,
			                       this,
			                       m_engine.m_window,
			                       filters,
			                       2,
		...
		void EditorManager::saveScene()
		{
			...
			if (m_sceneSerializer->saveScene(
			    m_sceneSerializer->getCurrentScenePath(), options))
			{
				AGNI_PRINT("[Editor] Scene saved: {}\n",
		...
		void EditorManager::saveSceneAs()
		{
			...
			saveSceneAsCallback, this, m_engine.m_window, filters, 2, nullptr);
