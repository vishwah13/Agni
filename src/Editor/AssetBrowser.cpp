...
namespace agni
{
	namespace editor
	{
		...
		static void assetBrowserFileWatchCallback(dmon_watch_id watch_id,
		                                          dmon_action   action,
		                                          const char*   rootdir,
		                                          const char*   filepath,
		...
		void AssetBrowser::renderToolbar()
		{
			...
			ImGui::SliderFloat(
			"##ThumbnailSize", &m_thumbnailSize, 60.0f, 150.0f, "%.0f");
		...
		void AssetBrowser::renderAssetGrid()
		{
			...
			{
				ImGui::TextDisabled("No assets loaded. Drag and drop .gltf or "
				                    ".glb files to load them.");
			}
			else
			{
		...
		void
		AssetBrowser::renderAssetThumbnail(const std::string&          name,
		                                   std::shared_ptr<LoadedGLTF> asset)
		{
			...
			ImVec4 thumbColor =
			ImVec4(0.2f, 0.2f, 0.25f, 1.0f); // Surface color
			...
			if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_None))
			{
				...
				"ASSET_GLTF", name.c_str(), name.size() + 1);
		...
		void AssetBrowser::renderFileThumbnail(FileNode& file, float size)
		{
			...
			if (ImGui::BeginDragDropSource(ImGuiDragDropFlags_None))
			{
				...
				ImGui::SetDragDropPayload(
				"ASSET_FILE", pathStr.c_str(), pathStr.size() + 1);

				// Drag preview
				ImGui::Text(
				"%s %s", getFileIcon(file.fullPath, false), file.name.c_str());
		...
		void
		AssetBrowser::scanDirectoryRecursive(const std::filesystem::path& dir,
		                                     FileNode&                    node)
		{
			...
			std::sort(node.children.begin(),
			          node.children.end(),
			          [](const std::unique_ptr<FileNode>& a,
			             const std::unique_ptr<FileNode>& b)
			          {
		...
		const char* AssetBrowser::getFileIcon(const std::filesystem::path& path,
		                                      bool isDirectory)
		{
			if (isDirectory)
		...
		void AssetBrowser::handleExternalDrop(
		const std::filesystem::path& externalPath)
		{
			// Determine target folder (use selected folder or default to
			// assets/models/)
			std::filesystem::path targetDir =
			m_selectedFolder && m_selectedFolder->isDirectory
			...
			targetDir / externalPath.filename();
