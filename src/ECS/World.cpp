...
namespace agni
{
	namespace ecs
	{
		    ...
		    m_prefabManager(std::make_unique<PrefabManager>(*this))
		{
			...
			// Note: PrefabManager built-in prefabs are registered after mesh
			// provider is set (done in AgniEngine after primitives are loaded)
		}
		...
		void World::registerSystems()
		{
			...
			m_world
			.system<TransformComponent, SceneNodeComponent>(
			"TransformHierarchy")
			.kind(flecs::PreUpdate)
			.each(
			[this](TransformComponent& transform, SceneNodeComponent& node)
			{
				if (!node.dirtyWorld)
					return;
				...
				if (node.parent != NULL_ENTITY)
				{
					...
					if (parentEntity.is_valid())
					{
						...
						parentEntity.try_get<TransformComponent>();
						if (parentTransform)
						{
				...
				parentWorld * transform.localTransform;
		...
		{
			flecs::entity entity =
			name ? m_world.entity(name) : m_world.entity();
		...
		void World::setLocalTransform(EntityHandle     entity,
		...
		void World::setParent(EntityHandle child, EntityHandle parent)
		{
			...
			SceneNodeComponent& childNode =
			childEntity.ensure<SceneNodeComponent>();
			...
			if (childNode.parent != NULL_ENTITY)
			{
				...
				{
					SceneNodeComponent& oldParentNode =
					oldParent.ensure<SceneNodeComponent>();
			...
			if (parentEntity.is_valid())
			{
				...
				{
					SceneNodeComponent& parentNode =
					parentEntity.ensure<SceneNodeComponent>();
		...
		void World::clearAllEntities()
		{
			...
			[](flecs::entity e, TransformComponent&) { e.destruct(); });
