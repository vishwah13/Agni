...
void ThreadPool::Initialize()
{
	...
	// This is good for mixed CPU/GPU workloads where we don't want to starve the main thread
	...
	std::cout << "[ThreadPool] Initialized with " << threadCount << " worker threads\n";
}
...
void ThreadPool::ParallelLoop(std::function<void(uint32_t start, uint32_t end)>&& function,
                              uint32_t                                            workTotal)
{
	if (workTotal == 0)
	{
	...
	// If we're already on a worker thread, just execute sequentially to avoid deadlock
	if (isWorkerThread)
	{
	...
	for (uint32_t i = 0; i < numTasks; i++)
	{
		...
		AddTask([&function, start, end, &completedTasks, &completionMutex, &completionCondition]()
		{
			function(start, end);
	...
	completionCondition.wait(lock, [&]() { return completedTasks >= numTasks; });
}
